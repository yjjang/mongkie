...
.. important::
    ...
    * Expression log2FCs of each gene in normal vs. tumor conditions (:download:`tcga_gbm_exp_log2fc.csv </assets/tcga_gbm_exp_log2fc.csv>`).
    * Expression means of each gene in 4 GBM subtypes (:download:`tcga_gbm_exp_subtypes.csv </assets/tcga_gbm_exp_subtypes.csv>`).
    * Expression correlations between each pair of genes.
    * Vertices and edges in a sub-network of altered (somatic mutation or CNV) genes (:download:`tcga_gbm_vertices.csv </assets/tcga_gbm_vertices.csv>` and :download:`tcga_gbm_edges.csv </assets/tcga_gbm_edges.csv>`).
    
    You can download the R script and input data files from `here <http://mongkie.readthedocs.org/en/latest/_downloads/assets.zip>`_.
